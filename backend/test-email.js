...
const runTest = async () => {
  console.log("🚀 Initializing SendGrid Test Protocol...");
  ...
  try {
    ...
    console.log(
      "✅ SendGrid commands dispatched successfully! Check your inbox.",
    );

    // process.exit() is intentionally omitted to allow the Node.js event loop
    // to close the HTTP sockets gracefully without throwing a Windows assertion error.
  } catch (error) {
    console.error(
      "❌ Test Failed. Check your SendGrid API Key and Verified Sender configuration:",
      error,
    );
  }
};
